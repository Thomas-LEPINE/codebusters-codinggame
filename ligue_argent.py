...
def call_allies_if_interesting(buster, ghosts):
    ...
    if len(ghosts) > 0:
        ...
        elif target_ghost["state"] > 5 and get_dist(buster, target_ghost) < 2200:
...
def default_move(i, buster, ghosts):
    ...
    elif len(available_known_ghosts) > i:
        target_ghost = sorted(available_known_ghosts, key=lambda ghost: ghost["state"])[i]
